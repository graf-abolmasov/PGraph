...
CREATE  TABLE IF NOT EXISTS `graph4`.`graphtop` (
  ...
  `SPRED` VARCHAR(8) NOT NULL ,
  `SMSG` VARCHAR(8) NOT NULL ,
...
CREATE  TABLE IF NOT EXISTS `graph4`.`gsp_shab` (
  ...
  `NLEX` INT(11) NULL DEFAULT NULL ,
  `TIPLEX` CHAR(2) NULL DEFAULT NULL ,
  `LEXEM` VARCHAR(255) NULL DEFAULT NULL ,
  PRIMARY KEY (`PROJECT_ID`,`NLEX`,`TIPLEX`,`LEXEM`) )
ENGINE = InnoDB
DEFAULT CHARACTER SET = cp1251;

 load data infile 'gsp_shab.txt' into table gsp_shab character set cp1251;
