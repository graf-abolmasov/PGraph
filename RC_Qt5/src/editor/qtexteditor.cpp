...
void QTextEditor::setupFileActions()
{
...
void QTextEditor::setupEditActions()
{
    QToolBar *tb = new QToolBar(this);
...
void QTextEditor::setupTextActions()
{
    QToolBar *tb = new QToolBar(this);
...
bool QTextEditor::load(const QString &f)
{
    if (!QFile::exists(f))
        return false;
    QFile file(f);
...
void QTextEditor::setCurrentFileName(const QString &fileName)
{
...
void QTextEditor::filePrintPreview()
{
#ifndef QT_NO_PRINTER
    QPrinter printer(QPrinter::HighResolution);
...
void QTextEditor::printPreview(QPrinter *printer)
{
#ifdef QT_NO_PRINTER
    Q_UNUSED(printer);
#else
...
void QTextEditor::textUnderline()
{
    QTextCharFormat fmt;
...
void QTextEditor::textFamily(const QString &f)
{
    QTextCharFormat fmt;
    fmt.setFontFamily(f);
    mergeFormatOnWordOrSelection(fmt);
...
void QTextEditor::textSize(const QString &p)
{
...
void QTextEditor::textStyle(int styleIndex)
{
...
void QTextEditor::textAlign(QAction *a)
{
...
void QTextEditor::currentCharFormatChanged(const QTextCharFormat &format)
{
    fontChanged(format.font());
    colorChanged(format.foreground().color());
}

void QTextEditor::cursorPositionChanged()
{
    alignmentChanged(textEdit->alignment());
}

void QTextEditor::clipboardDataChanged()
{
#ifndef QT_NO_CLIPBOARD
    if (const QMimeData *md = QApplication::clipboard()->mimeData())
        actionPaste->setEnabled(md->hasText());
...
void QTextEditor::mergeFormatOnWordOrSelection(const QTextCharFormat &format)
{
    QTextCursor cursor = textEdit->textCursor();
...
void QTextEditor::fontChanged(const QFont &f)
{
...
void QTextEditor::colorChanged(const QColor &c)
{
    QPixmap pix(16, 16);
    pix.fill(c);
...
void QTextEditor::alignmentChanged(Qt::Alignment a)
{
    if (a & Qt::AlignLeft)
